...
class OrderProcessingController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if ($request->ajax() || $request->has('ajax')) {
            return view('tenant.processing.orders.partials.orders-content', compact('orders', 'counts', 'amounts'));
        }

        return view('tenant.processing.orders.index', compact('orders', 'counts', 'amounts'));
    }
    ...
    public function readyToShip(Order $order): RedirectResponse
    {
        try {
            ...
            return back()->with('success', 'Order marked as Ready to Parcel.');
        } catch (Exception $e) {
            return back()->with('error', 'Error updating order: ' . $e->getMessage());
        }
    }
