...
class StockTransferController extends Controller
{
    public function index()
    {
        ...
        // We could load products via AJAX, but for simplicity let's load some
        $products = Product::where('is_active', true)->limit(50)->get();

        return view('tenant.stock_transfers.index', compact('warehouses', 'products'));
    }
        ...
        try {
            DB::transaction(function () use ($validated) {
                ...
                InventoryMovement::create([
                    ...
                    'type' => 'transfer_out',
                    'quantity' => -$quantity,
                    'reference_id' => null, // Could be null or a transfer ID if we had a Transfer model
                ...
                // Lock Destination for Safety (though update handles it, good for consistency)
                // Since we might have just created it, it might not be locked in the same way, but firstOrCreate inside transaction is generally okay.
                // To be strictly safe we could re-fetch with lock, but for now:
