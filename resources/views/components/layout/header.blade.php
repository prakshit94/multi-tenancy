        ...
        @can('customers view')
            @if(!$hideSearch)
                <div
                    ...
                    <!-- FINAL THEME MATCHED MODAL -->
<template x-teleport="body">
    <div x-show="showModal"
        x-transition.opacity
        class="fixed inset-0 z-[9999] flex items-center justify-center bg-background/80 backdrop-blur-sm p-4"
        style="display: none;">

        <div class="relative w-full max-w-2xl">

            <!-- Modal Card -->
            <div class="relative bg-card/90 backdrop-blur-xl border border-border/40 rounded-3xl shadow-2xl max-h-[90vh] flex flex-col overflow-hidden"
                @click.away="showModal = false">

                <!-- Subtle Top Accent -->
                <div class="h-1 bg-gradient-to-r from-primary/60 via-primary/20 to-transparent"></div>

                <!-- Close -->
                <button @click="showModal = false"
                    class="absolute top-5 right-5 z-50 size-9 flex items-center justify-center rounded-xl bg-muted/40 hover:bg-muted transition">
                    ✕
                </button>

                <!-- Content -->
                <div class="flex-1 overflow-y-auto px-6 py-7 space-y-8">

                    <!-- Title -->
                    <div class="space-y-1">
                        <h2 class="text-xl font-bold text-foreground">Add Customer</h2>
                        <p class="text-xs text-muted-foreground">Enter customer details</p>
                    </div>
                    ...
                    <section class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <input type="text" x-model="newCustomer.first_name" placeholder="First Name" class="input">
                            <input type="text" x-model="newCustomer.middle_name" placeholder="Middle Name" class="input">
                            <input type="text" x-model="newCustomer.last_name" placeholder="Last Name" class="input">
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <input type="text" x-model="newCustomer.mobile" maxlength="10" placeholder="Mobile"
                                class="input-lg">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <template x-for="type in ['farmer','buyer','dealer','vendor']">
                                <button @click="newCustomer.type = type"
                                    :class="newCustomer.type === type 
                                        ? 'bg-primary text-primary-foreground shadow-sm' 
                                        : 'bg-muted text-muted-foreground hover:bg-muted/70'"
                                    class="px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wide transition">
                    ...
                    <section class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div class="relative">
                                ...
                                <div x-show="addressSuggestions.length && activeLookupField === 'village'"
                                    ...
                                    <template x-for="item in addressSuggestions">
                                        <div @click="fillAddress(item.data)" class="dropdown-item"
                            ...
                            <input type="text" x-model="newCustomer.post_office" placeholder="Post Office" class="input">
                            <input type="text" x-model="newCustomer.taluka" placeholder="Taluka" class="input">
                            <input type="text" x-model="newCustomer.district" placeholder="District" class="input">
                            <input type="text" x-model="newCustomer.state" placeholder="State" class="input">
                        </div>
                    ...
                    <div class="pt-2">
                        <button @click="createCustomer()" :disabled="saving"
                            class="w-full h-11 rounded-xl bg-primary text-primary-foreground text-sm font-bold tracking-wide hover:opacity-90 active:scale-[0.98] transition flex items-center justify-center gap-2">
                    ...
                    <!-- Error -->
                    <div x-show="error"
                        class="text-sm text-rose-600 bg-rose-500/10 border border-rose-500/20 p-3 rounded-xl">
