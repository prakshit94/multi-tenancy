...
@section('content')
<div id="suppliers-page-wrapper" class="flex flex-1 flex-col space-y-8 p-8 animate-in fade-in duration-500">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div class="space-y-1">
            <h1 class="text-3xl font-bold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">Suppliers</h1>
            <p class="text-muted-foreground text-sm">Manage suppliers and vendor relationships.</p>
        </div>
        <div class="flex items-center p-1 bg-muted/50 rounded-xl border border-border/50 backdrop-blur-sm">
            <a href="{{ route('central.suppliers.index') }}" class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === null ? 'bg-background text-foreground shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-foreground hover:bg-background/50' }}">
                All Suppliers
            </a>
            <div class="w-px h-4 bg-border/40 mx-1"></div>
            <a href="{{ route('central.suppliers.index', ['status' => 'active']) }}" class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === 'active' ? 'bg-background text-emerald-600 shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-emerald-600 hover:bg-background/50' }}">
                Active
            </a>
        </div>
    </div>

    <div id="suppliers-table-container" x-data="{ selected: [] }">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-1.5 rounded-2xl">
            ...
            <a href="{{ route('central.suppliers.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20 hover:bg-primary/90 hover:scale-[1.02] active:scale-95 transition-all duration-200">
        ...
        <div class="rounded-2xl border border-border/40 bg-card/50 backdrop-blur-xl shadow-sm overflow-hidden relative">
                        ...
                        <tr class="border-b border-border/40 transition-colors hover:bg-muted/30 data-[state=selected]:bg-muted bg-muted/20">
                            <th class="h-12 w-[50px] px-6 text-left align-middle">
                                <div class="flex items-center">
                                    <input type="checkbox" class="h-4 w-4 rounded border-input text-primary focus:ring-primary/20 bg-background cursor-pointer transition-all checked:bg-primary checked:border-primary" @click="selected = $event.target.checked ? [{{ $suppliers->pluck('id')->join(',') }}] : []">
                                </div>
                            </th>
                            ...
                            <th class="h-12 px-6 text-left align-middle font-medium text-muted-foreground/70 uppercase tracking-wider text-[11px]">Primary Crop</th>
                            <th class="h-12 px-6 text-left align-middle font-medium text-muted-foreground/70 uppercase tracking-wider text-[11px]">Verification</th>
                    ...
                    <tbody class="[&_tr:last-child]:border-0 text-sm">
                        ...
                        <tr class="group border-b border-border/40 transition-all duration-200 hover:bg-muted/40 data-[state=selected]:bg-muted/60">
                             <td class="p-6 align-middle">
                                <div class="flex items-center">
                                    <input type="checkbox" value="{{ $supplier->id }}" x-model="selected" class="h-4 w-4 rounded border-input text-primary focus:ring-primary/20 bg-background cursor-pointer transition-all checked:bg-primary checked:border-primary">
                                </div>
                            </td>
                            <td class="p-6 align-middle">
                                <div class="flex flex-col space-y-0.5">
                                    <span class="font-semibold text-foreground text-sm tracking-tight">{{ $supplier->company_name }}</span> <!-- Fixed: Use company_name not name -->
                                    <span class="text-xs text-muted-foreground font-mono">{{ $supplier->email ?? '-' }}</span>
                                </div>
                            </td>
                            <td class="p-6 align-middle">
                                <span class="text-sm">{{ $supplier->contact_name ?? '-' }}</span>
                            </td>
                            <td class="p-6 align-middle">
                                <span class="text-sm">{{ $supplier->primary_crop ?? '-' }}</span>
                            </td>
                            <td class="p-6 align-middle">
                                @if($supplier->verification_status == 'verified')
                                    <span class="inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800">Verified</span>
                                @elseif($supplier->verification_status == 'rejected')
                                    <span class="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800">Rejected</span>
                                @else
                                    <span class="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-800">Unverified</span>
                                @endif
                            </td>
                            <td class="p-6 align-middle">
                            ...
                            <td class="p-6 align-middle text-right">
                                 <div class="relative flex justify-end" x-data="{ open: false }" @click.away="open = false">
                                    ...
                                    <div x-show="open" class="absolute right-0 top-9 z-50 min-w-[180px] overflow-hidden rounded-xl border border-border/60 bg-popover/95 p-1 text-popover-foreground shadow-xl shadow-black/5 backdrop-blur-xl" style="display: none;">
                                        ...
                                        <a href="{{ route('central.suppliers.edit', $supplier) }}" class="flex w-full cursor-pointer select-none items-center gap-2 rounded-lg px-2 py-2 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground">
                                            Edit Details
                                        </a>
                                        <div class="my-1 h-px bg-border/50"></div>
                                        <form action="{{ route('central.suppliers.destroy', $supplier) }}" method="POST" onsubmit="return confirm('Delete supplier?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="p-16 text-center">
                                <div class="flex flex-col items-center justify-center text-muted-foreground/50">
                                    <p class="text-lg font-semibold text-foreground">No suppliers found</p>
                                    <p class="text-sm mt-1">Add your first supplier.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
            ...
            <div class="border-t border-border/40 p-4 bg-muted/20 flex flex-col sm:flex-row items-center justify-between gap-4">
                 <div class="text-xs text-muted-foreground px-2">Page <span class="font-medium text-foreground">{{ $suppliers->currentPage() }}</span> of <span class="font-medium">{{ $suppliers->lastPage() }}</span></div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function loadContent(url, pushState = true) {
            ...
            try {
                ...
                const newContent = doc.getElementById('suppliers-table-container');
                if (newContent) {
