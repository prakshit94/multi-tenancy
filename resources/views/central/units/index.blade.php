...
@section('content')
    <div id="units-page-wrapper" class="flex flex-1 flex-col space-y-8 p-8 animate-in fade-in duration-500">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="space-y-1">
                <h1
                    ...
                    Units</h1>
                <p class="text-muted-foreground text-sm">Manage product measurement units.</p>
            </div>
            <div class="flex items-center p-1 bg-muted/50 rounded-xl border border-border/50 backdrop-blur-sm">
                <a href="{{ route('central.units.index') }}"
                ...
                <a href="{{ route('central.units.index', ['status' => 'active']) }}"
        ...
        <div id="units-table-container" x-data="{ selected: [] }">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-1.5 rounded-2xl">
                ...
                <a href="{{ route('central.units.create') }}"
                                ...
                                <th class="h-12 w-[50px] px-6 text-left align-middle">
                                    <div class="flex items-center">
                                        <input type="checkbox"
                                            ...
                                            @click="selected = $event.target.checked ? [{{ $units->pluck('id')->join(',') }}] : []">
                                    </div>
                                </th>
                                <th
                            ...
                            @forelse($units as $unit)
                                <tr
                                    ...
                                    <td class="p-6 align-middle">
                                        <div class="flex items-center">
                                            <input type="checkbox" value="{{ $unit->id }}" x-model="selected"
                                                class="h-4 w-4 rounded border-input text-primary focus:ring-primary/20 bg-background cursor-pointer transition-all checked:bg-primary checked:border-primary">
                                        </div>
                                    </td>
                                    <td class="p-6 align-middle">
                                        <span
                                            class="font-semibold text-foreground text-sm tracking-tight">{{ $unit->name }}</span>
                                    </td>
                                    <td class="p-6 align-middle">
                                        @if($unit->is_active)
                                            <span
                                        ...
                                        <div class="relative flex justify-end" x-data="{ open: false }"
                                            ...
                                            <div x-show="open"
                                                ...
                                                <div
                                                    ...
                                                    Manage</div>
                                                <a href="{{ route('central.units.edit', $unit) }}"
                                                ...
                                                <form action="{{ route('central.units.destroy', $unit) }}" method="POST"
                                ...
                                <tr>
                                    <td colspan="4" class="p-16 text-center">
                                        <div class="flex flex-col items-center justify-center text-muted-foreground/50">
                                            <p class="text-lg font-semibold text-foreground">No units found</p>
                                            <p class="text-sm mt-1">Add your first unit.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                @if($units->hasPages())
                    <div
                        ...
                        <div class="text-xs text-muted-foreground px-2">Page <span
                                class="font-medium text-foreground">{{ $units->currentPage() }}</span> of <span
                                class="font-medium">{{ $units->lastPage() }}</span></div>
                        <div>{{ $units->links() }}</div>
                    </div>
                @endif
            </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function loadContent(url, pushState = true) {
                ...
                try {
                    ...
                    const newContent = doc.getElementById('units-table-container');
                    if (newContent) {
