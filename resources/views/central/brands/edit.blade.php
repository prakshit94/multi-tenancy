...
@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold font-heading tracking-tight text-foreground">Edit Brand</h1>
                <p class="text-muted-foreground text-sm">Update details for {{ $brand->name }}.</p>
            </div>
        </div>

        <!-- Form Card -->
        <div class="rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm shadow-sm overflow-hidden p-6">
            <form action="{{ route('central.brands.update', $brand) }}" method="POST" class="space-y-6"
                x-data="{ name: '{{ addslashes($brand->name) }}', slug: '{{ $brand->slug }}' }">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div class="space-y-2">
                        <label
                            class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Brand
                            Name</label>
                ...
                <div class="space-y-2">
                    <label
                        class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Logo
                        URL</label>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="meta_description" rows="3"
                        class="flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">{{ $brand->meta_description }}</textarea>
                </div>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <input type="checkbox" name="is_active" value="1" id="is_active"
                        class="h-4 w-4 rounded border-primary text-primary shadow focus:ring-primary" {{ $brand->is_active ? 'checked' : '' }}>
                    <label for="is_active"
                        class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer">Active
                ...
                <div class="pt-4 flex justify-end gap-3">
                    <x-ui.button type="button" variant="outline"
                        href="{{ route('central.brands.index') }}">Cancel</x-ui.button>
