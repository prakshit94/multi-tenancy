...
<template x-teleport="body">
   <div x-show="showTaggingModal"
      ...
      <div
         class="w-full max-w-lg rounded-2xl border border-border bg-white dark:bg-zinc-900 shadow-xl overflow-hidden flex flex-col max-h-[90vh]"
         ...
         <div class="flex-1 overflow-y-auto p-6 custom-scrollbar">
            ...
            <!-- HEADER (Simple) -->
            <div class="mb-6">
               <h3 class="text-lg font-semibold">Interaction Outcome</h3>
            </div>
            ...
            <div class="space-y-6">
                  ...
                  <div class="grid grid-cols-2 gap-3" x-show="!showManageOutcomes">
                     <template x-for="opt in outcomes" :key="opt.id">
                        <button @click="tagOutcome = opt.name"
                           :class="tagOutcome === opt.name 
                              ? 'bg-primary text-primary-foreground border-primary' 
                              : 'bg-muted hover:bg-muted/70 border-border text-muted-foreground'"
                           class="p-3 rounded-lg text-xs font-semibold border transition-all text-center">
                     ...
                     <button @click="showManageOutcomes = true" x-show="isSuperAdmin"
                        class="p-3 rounded-lg text-xs font-semibold border border-dashed border-primary/40 text-primary hover:bg-primary/5 transition-all flex items-center justify-center gap-1">
                        + Manage
                     </button>
                  </div>

                  <!-- MANAGEMENT -->
                  <div class="space-y-3" x-show="showManageOutcomes" x-transition>
                     <div class="flex gap-2">
                        <input type="text" x-model="newOutcomeName"
                           placeholder="New Outcome Name"
                           class="flex-1 bg-muted border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary">

                        <button @click="addOutcome()"
                           :disabled="!newOutcomeName.trim() || outcomeLoading"
                           class="px-4 rounded-lg bg-primary text-primary-foreground text-xs font-semibold disabled:opacity-50">
                           Add
                        </button>
                     </div>
                     ...
                     <div class="max-h-40 overflow-y-auto space-y-2 pr-1">
                        <template x-for="opt in outcomes" :key="opt.id">
                           <div class="flex justify-between items-center p-2 bg-muted rounded-md border border-border">
                              <span class="text-xs font-medium" x-text="opt.name"></span>
               ...
               <div class="space-y-2">
                  ...
                  <textarea x-model="tagNotes"
                     class="w-full bg-muted border border-border rounded-lg p-3 text-sm focus:outline-none focus:ring-1 focus:ring-primary min-h-[90px]"
         ...
         <div class="p-4 border-t border-border flex gap-3">
            ...
            <button @click="showTaggingModal = false" :disabled="taggingLoading"
               class="flex-1 h-11 rounded-lg border border-border text-xs font-semibold hover:bg-muted transition">
               Cancel
            </button>

            <button @click="submitTagging()" :disabled="taggingLoading"
               class="flex-1 h-11 rounded-lg bg-primary text-primary-foreground text-xs font-semibold disabled:opacity-50 flex items-center justify-center gap-2">

               <span x-show="taggingLoading"
                  class="animate-spin size-3 border-2 border-white/30 border-t-white rounded-full"></span>

               <span x-text="taggingLoading ? 'Saving...' : 'Confirm'"></span>
            </button>
...
<div x-show="activeTab === 'timeline'"
   ...
   <div class="max-w-4xl mx-auto space-y-10 relative">
      ...
      <!-- Line -->
      <div class="absolute left-[19px] top-4 bottom-4 w-[2px] bg-border"></div>

      <!-- Loading -->
      <template x-if="activityLoading">
         <div class="py-20 text-center">
            <div class="size-10 border-2 border-primary/30 border-t-primary rounded-full animate-spin mx-auto mb-3"></div>
      ...
      <template x-for="event in activityEvents" :key="event.id + event.kind">
         <div class="relative pl-14">
            ...
            <!-- ICON -->
            <div class="absolute left-0 top-1 size-9 rounded-full flex items-center justify-center text-white"
            ...
            <!-- CARD -->
            <div class="bg-white dark:bg-zinc-900 border border-border rounded-xl p-5">
               ...
               <template x-if="event.kind === 'order'">
                  <div>
                     ...
                     <div class="flex justify-between mb-3">
                        <div>
                           <p class="text-xs text-muted-foreground" x-text="event.placed_at"></p>
                           <h4 class="font-bold" x-text="event.order_number"></h4>
                        </div>
                        <p class="font-bold text-lg" x-text="'Rs ' + event.grand_total"></p>
                     </div>

                     <p class="text-xs text-muted-foreground mb-2" x-text="event.status"></p>
                  ...
                  <div>
                     ...
                     <div class="flex justify-between mb-3">
                        <div class="flex items-center gap-2">
                           <span class="text-[10px] px-2 py-1 rounded bg-emerald-100 text-emerald-600 font-semibold"
                              x-text="event.interaction_type || 'Interaction'"></span>
                     ...
                     <!-- NOTES -->
                     <div class="text-sm bg-muted p-3 rounded-lg mb-3"
                        x-text="event.notes || 'No notes provided'">
                     </div>

                     <!-- 🔥 NEW: COMPLETE TAG DETAILS -->
                     <div class="grid grid-cols-2 gap-3 text-xs">

                        <div>
                           <p class="text-muted-foreground">Outcome</p>
                           <p class="font-semibold"
                              x-text="event.outcome || event.tag_outcome || '-'"></p>
                        </div>

                        <div>
                           <p class="text-muted-foreground">Status</p>
                           <p class="font-semibold"
                              x-text="event.status || '-'"></p>
                        </div>

                        <div>
                           <p class="text-muted-foreground">Next Follow-up</p>
                           <p class="font-semibold"
                              x-text="event.next_followup || '-'"></p>
                        </div>

                        <div>
                           <p class="text-muted-foreground">Duration</p>
                           <p class="font-semibold"
                              x-text="event.duration || '-'"></p>
                        </div>
                           ...
                           <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                              ...
                              <div x-show="order.is_future_order"
                                 ...
                                 <p class="text-[10px] text-muted-foreground italic">Order will be
                                    processed at the selected time.
                                 </p>
                              </div>
                           </div>
                        </div>
