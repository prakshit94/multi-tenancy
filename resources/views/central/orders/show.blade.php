        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-100">
                    ...
                    {{-- Cancel (Separate) --}}
                     @if(!in_array($order->status, ['completed', 'cancelled', 'returned', 'shipped', 'in_transit', 'delivered']))
                        @can('orders cancel')
                            <form action="{{ route('central.orders.update-status', $order) }}" method="POST"
                ...
                <div class="flex items-center gap-2" x-data="{}">
                    @if($order->invoices->isNotEmpty())
                        ...
                        @can('invoices view')
                            <a href="{{ route('central.invoices.pdf', $invoice) }}" 
                               ...
                               class="inline-flex items-center px-3 py-1.5 border border-gray-200 shadow-sm text-xs font-bold rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none transition-all gap-1" title="Print Invoice">
                    ...
                    @can('orders-receipt view')
                        <a href="{{ route('central.orders.receipt', $order) }}" 
                           ...
                           class="inline-flex items-center px-3 py-1.5 border border-gray-200 shadow-sm text-xs font-bold rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none transition-all gap-1" title="Print Receipt">
