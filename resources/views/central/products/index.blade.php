                    ...
                    <div class="bg-white dark:bg-zinc-900 w-full max-w-4xl rounded-[40px] shadow-[0_0_100px_rgba(0,0,0,0.5)] border border-white/10 overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 fade-in duration-500 ease-out"
                        ...
                        <div class="px-10 pt-10 pb-6 relative overflow-hidden">
                            ...
                            <div class="relative z-10 flex justify-between items-start">
                                <div>
                                    <h3 class="font-black text-3xl tracking-tighter text-gray-900" x-text="viewingProduct?.name"></h3>
                                    <div class="flex items-center gap-3 mt-2">
                                        <span class="px-3 py-1 rounded-full bg-gray-100 text-[10px] font-black uppercase tracking-widest text-gray-500 border border-gray-200" x-text="viewingProduct?.sku"></span>
                                        <span class="px-3 py-1 rounded-full bg-indigo-50 text-[10px] font-black uppercase tracking-widest text-indigo-600 border border-indigo-100" x-text="viewingProduct?.category?.name || 'Uncategorized'"></span>
                                        <template x-if="viewingProduct?.is_active">
                                            <span class="px-3 py-1 rounded-full bg-emerald-50 text-[10px] font-black uppercase tracking-widest text-emerald-600 border border-emerald-100">Active</span>
                                        </template>
                                        <template x-if="!viewingProduct?.is_active">
                                            <span class="px-3 py-1 rounded-full bg-red-50 text-[10px] font-black uppercase tracking-widest text-red-600 border border-red-100">Inactive</span>
                                        </template>
                                    </div>
                                </div>
                        ...
                        <div class="px-10 pb-10 overflow-y-auto custom-scrollbar">
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-10">
                                ...
                                <div class="md:col-span-4">
                                    <div class="aspect-square rounded-[32px] overflow-hidden border border-gray-100 bg-gray-50 shadow-inner group">
                                    ...
                                    <div class="mt-6 p-6 rounded-[32px] bg-gray-50 border border-gray-100 space-y-4">
                                        <div class="flex justify-between items-center">
                                            ...
                                            <span class="text-xl font-black text-indigo-600" x-text="formatPrice(viewingProduct?.price || 0)"></span>
                                        </div>
                                        <div class="flex justify-between items-center" x-show="viewingProduct?.mrp > viewingProduct?.price">
                                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">MRP</span>
                                            <span class="text-sm font-bold text-gray-400 line-through" x-text="formatPrice(viewingProduct?.mrp || 0)"></span>
                                        </div>
                                        <div class="flex justify-between items-center" x-show="viewingProduct?.stock_on_hand !== null">
                                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Stock Status</span>
                                            <span class="text-sm font-black" :class="viewingProduct?.stock_on_hand > 10 ? 'text-emerald-600' : 'text-amber-600'" x-text="viewingProduct?.manage_stock ? parseFloat(viewingProduct?.stock_on_hand) + ' ' + (viewingProduct?.unit_type || '') : 'Not Tracked'"></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="md:col-span-8 space-y-8">
                                    <!-- Description -->
                                    <div class="space-y-3">
                                        <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Product Overview</h4>
                                        <div class="text-sm leading-relaxed text-gray-600 font-medium whitespace-pre-line" x-text="viewingProduct?.description || 'No description available.'"></div>
                                    </div>

                                    <!-- Specifications -->
                                    <div class="grid grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                                        <div class="space-y-1">
                                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Brand</span>
                                            <p class="text-sm font-bold text-gray-900" x-text="viewingProduct?.brand?.name || 'N/A'"></p>
                                        </div>
                                        <div class="space-y-1">
                                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Packing Size</span>
                                            <p class="text-sm font-bold text-gray-900" x-text="viewingProduct?.packing_size || 'N/A'"></p>
                                        </div>
                                        <div class="space-y-1">
                                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">HSN Code</span>
                                            <p class="text-sm font-bold text-gray-900" x-text="viewingProduct?.hsn_code || 'N/A'"></p>
                                        </div>
                                        <div class="space-y-1">
                                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Product Type</span>
                                            <p class="text-sm font-bold text-gray-900" x-text="viewingProduct?.type || 'Standard'"></p>
                                        </div>
                                    </div>
                                    ...
                                    <template x-if="viewingProduct?.technical_name || viewingProduct?.harvest_date">
                                        <div class="p-6 rounded-[32px] bg-indigo-50/50 border border-indigo-100 space-y-4">
                                            <h4 class="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">Agri Details</h4>
                                            <div class="grid grid-cols-2 gap-4">
                                                <div x-show="viewingProduct?.technical_name">
                                                    <span class="text-[9px] font-black text-gray-400 uppercase">Technical Name</span>
                                                    <p class="text-xs font-bold text-gray-700 mt-0.5" x-text="viewingProduct?.technical_name"></p>
                                                </div>
                                                <div x-show="viewingProduct?.application_method">
                                                    <span class="text-[9px] font-black text-gray-400 uppercase">Application</span>
                                                    <p class="text-xs font-bold text-gray-700 mt-0.5" x-text="viewingProduct?.application_method"></p>
                                                </div>
                                                <div x-show="viewingProduct?.harvest_date">
                                                    <span class="text-[9px] font-black text-gray-400 uppercase">Harvest Date</span>
                                                    <p class="text-xs font-bold text-gray-700 mt-0.5" x-text="formatDate(viewingProduct?.harvest_date)"></p>
                                                </div>
                                                <div x-show="viewingProduct?.expiry_date">
                                                    <span class="text-[9px] font-black text-gray-400 uppercase">Expiry Date</span>
                                                    <p class="text-xs font-bold text-gray-700 mt-0.5" x-text="formatDate(viewingProduct?.expiry_date)"></p>
                                                </div>
                                            </div>
                                        </div>
                                    </template>
