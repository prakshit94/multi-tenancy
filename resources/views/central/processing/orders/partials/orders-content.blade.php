...
<div class="flex flex-col space-y-4 mb-8">
   ...
   <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-6 p-4 bg-gray-50/50 rounded-[40px] border border-gray-100/50 backdrop-blur-sm shadow-sm ring-1 ring-black/5">
        ...
        <div class="flex flex-wrap items-center gap-2">
         ...
         ', 'color' => 'blue'],
         ['label' => 'PROCESSING', 'status' => 'processing', 'icon' => '
         <path d="M12 2 2 7v10l10 5 10-5V7Z"/>
         <path d="m2 7 10 5 10-5"/>
         <path d="M12 22V12"/>
         ', 'color' => 'purple'],
         ['label' => 'READY TO SHIP', 'status' => 'ready_to_ship', 'icon' => '
         <path d="M21 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v3m18 0v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8m18 0-9 6-9-6"/>
         ', 'color' => 'emerald'],
         ['label' => 'SHIPPED', 'status' => 'shipped', 'icon' => '
         <path d="M10 17h4V5H2v12h3m0 0a2 2 0 1 0 4 0 2 2 0 1 0-4 0m10 0a2 2 0 1 0 4 0 2 2 0 1 0-4 0M13 5h9l-1 7h-8z"/>
         ', 'color' => 'indigo'],
         ['label' => 'DELIVERD', 'status' => 'delivered', 'icon' => '
         <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 1 1-7.6-11.7 8.38 8.38 0 0 1 3.8.9L21 2z"/>
      ...
      <div class="flex items-center gap-3">
         <form id="filter-form" @submit.prevent="performFilter()" 
            ...
            <div class="relative min-w-[160px]" x-data="{ 
               ...
               <button type="button" @click="open = !open" class="w-full h-11 px-4 bg-white border border-gray-200 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-between gap-2 shadow-sm hover:border-primary transition-all">
                  ...
                  <svg class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="m6 9 6 6 6-6"/>
                  </svg>
               </button>
                     ...
                     <template x-for="opt in filteredOptions" :key="opt">
                        <button type="button" @click="toggle(opt)" class="w-full px-4 py-2 text-left text-[10px] font-bold uppercase text-gray-600 hover:bg-primary/5 hover:text-primary transition-all flex items-center justify-between" :class="selected.includes(opt) ? 'bg-primary/10 text-primary' : ''">
                           ...
                           <div class="w-3 h-3 rounded-sm border flex items-center justify-center transition-colors" :class="selected.includes(opt) ? 'border-primary bg-primary text-white' : 'border-gray-300 bg-white'">
                              <svg x-show="selected.includes(opt)" class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="4">
            ...
            <div class="relative min-w-[160px]" x-data="{ 
               ...
               $nextTick(() => { 
               $refs.productInput.value = this.selected.join(','); 
               $refs.productInput.dispatchEvent(new Event('change')); 
               performFilter(); 
               }); 
               },
               clear() {
               ...
               <button type="button" @click="open = !open"
                  class="w-full h-11 px-4 bg-white border border-gray-200 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-between gap-2 shadow-sm hover:border-primary transition-all">
                  <span x-text="selected.length > 0 ? selected.length + ' Selected' : 'Product'" class="truncate"></span>
                  <svg class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="m6 9 6 6 6-6"/>
                  </svg>
               </button>
               <div x-show="open" x-transition.origin.top 
                  class="absolute z-[110] left-0 right-0 mt-2 bg-white border border-gray-100 rounded-2xl shadow-2xl overflow-hidden max-h-72 flex flex-col">
                  <input type="text" x-model="search" placeholder="Search..." 
                     class="w-full px-3 py-2 text-[10px] font-bold border-b border-gray-50 bg-gray-50/50 outline-none uppercase">
                  <div class="overflow-y-auto flex-1 custom-scrollbar py-1">
                     <button type="button" @click="clear()" 
                        class="w-full px-4 py-2 text-left text-[10px] font-black uppercase text-gray-400 hover:bg-gray-50 flex justify-between items-center bg-gray-50/30">
                     All Products
                     <span x-show="selected.length > 0" x-text="selected.length"
                        class="bg-gray-200 text-gray-600 px-1.5 py-0.5 rounded text-[9px] font-bold"></span>
                     </button>
                     <template x-for="opt in filteredOptions" :key="opt">
                        <button type="button" @click="toggle(opt)"
                           class="w-full px-4 py-2 text-left text-[10px] font-bold uppercase text-gray-600 hover:bg-primary/5 hover:text-primary transition-all flex items-center justify-between"
                           :class="selected.includes(opt) ? 'bg-primary/10 text-primary' : ''">
                           <span x-text="opt"></span>
                           <div class="w-3 h-3 rounded-sm border flex items-center justify-center transition-colors"
                              :class="selected.includes(opt) ? 'border-primary bg-primary text-white' : 'border-gray-300 bg-white'">
                              <svg x-show="selected.includes(opt)" class="w-2.5 h-2.5"
...
<div class="grid grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6">
   @php
   $stats = [
   ['label' => 'Confirmed', 'status' => 'confirmed', 'count' => $counts['confirmed'] ?? 0, 'color' => 'blue', 'icon' => '
   <circle cx="12" cy="12" r="10"/>
   <path d="m9 12 2 2 4-4"/>
   '],
   ['label' => 'Processing', 'status' => 'processing', 'count' => $counts['processing'] ?? 0, 'color' => 'purple', 'icon' => '
   <path d="m7.5 4.27 9 5.15"/>
   <path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/>
   <path d="m3.3 7 8.7 5 8.7-5"/>
   <path d="M12 22V12"/>
   '],
   ['label' => 'Ready to Ship', 'status' => 'ready_to_ship', 'count' => $counts['ready_to_ship'] ?? 0, 'color' => 'emerald', 'icon' => '
   <path d="M2 9h20"/>
   <path d="M4 9h2V5a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v4h2"/>
   <path d="M22 9v10a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V9Z"/>
   <path d="M12 12v3"/>
   <path d="M12 15h3"/>
   '],
   ['label' => 'Dispatched', 'status' => 'shipped', 'count' => $counts['shipped'] ?? 0, 'color' => 'indigo', 'icon' => '
   <path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"/>
   <path d="M15 18H9"/>
   <path d="M19 18h2a1 1 0 0 0 1-1v-5l-4-4h-4"/>
   <circle cx="7" cy="18" r="2"/>
   <circle cx="17" cy="18" r="2"/>
   '],
   ['label' => 'Deliverd', 'status' => 'delivered', 'count' => $counts['delivered'] ?? 0, 'color' => 'green', 'icon' => '
