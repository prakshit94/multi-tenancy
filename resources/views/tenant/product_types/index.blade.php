...
@section('content')
    <div id="product-types-page-wrapper" class="flex flex-1 flex-col space-y-8 p-8 animate-in fade-in duration-500">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="space-y-1">
                <h1
                    ...
                    Product Types</h1>
                <p class="text-muted-foreground text-sm">Manage product types (e.g. Simple, Variable, Service).</p>
            </div>
            <div class="flex items-center p-1 bg-muted/50 rounded-xl border border-border/50 backdrop-blur-sm">
                <a href="{{ route('tenant.product_types.index') }}"
                ...
                <a href="{{ route('tenant.product_types.index', ['status' => 'active']) }}"
        ...
        <div id="product-types-table-container" x-data="{ selected: [] }">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-1.5 rounded-2xl">
                ...
                <a href="{{ route('tenant.product_types.create') }}"
                                ...
                                <th class="h-12 w-[50px] px-6 text-left align-middle">
                                    <div class="flex items-center">
                                        <input type="checkbox"
                                            ...
                                            @click="selected = $event.target.checked ? [{{ $productTypes->pluck('id')->join(',') }}] : []">
                                    </div>
                                </th>
                                <th
                            ...
                            @forelse($productTypes as $pt)
                                <tr
                                    ...
                                    <td class="p-6 align-middle">
                                        <div class="flex items-center">
                                            <input type="checkbox" value="{{ $pt->id }}" x-model="selected"
                                                class="h-4 w-4 rounded border-input text-primary focus:ring-primary/20 bg-background cursor-pointer transition-all checked:bg-primary checked:border-primary">
                                        </div>
                                    </td>
                                    <td class="p-6 align-middle">
                                        <span
                                            class="font-semibold text-foreground text-sm tracking-tight">{{ $pt->name }}</span>
                                    </td>
                                    <td class="p-6 align-middle">
                                        @if($pt->is_active)
                                            <span
                                        ...
                                        <div class="relative flex justify-end" x-data="{ open: false }"
                                            ...
                                            <div x-show="open"
                                                ...
                                                <div
                                                    ...
                                                    Manage</div>
                                                <a href="{{ route('tenant.product_types.edit', $pt) }}"
                                                ...
                                                <form action="{{ route('tenant.product_types.destroy', $pt) }}" method="POST"
                                                    onsubmit="return confirm('Delete product type?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                ...
                                <tr>
                                    <td colspan="4" class="p-16 text-center">
                                        <div class="flex flex-col items-center justify-center text-muted-foreground/50">
                                            <p class="text-lg font-semibold text-foreground">No product types found</p>
                                            <p class="text-sm mt-1">Add your first product type.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                @if($productTypes->hasPages())
                    <div
                        ...
                        <div class="text-xs text-muted-foreground px-2">Page <span
                                class="font-medium text-foreground">{{ $productTypes->currentPage() }}</span> of <span
                                class="font-medium">{{ $productTypes->lastPage() }}</span></div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function loadContent(url, pushState = true) {
                ...
                try {
                    ...
                    const newContent = doc.getElementById('product-types-table-container');
                    if (newContent) {
