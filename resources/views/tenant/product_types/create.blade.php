...
@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold font-heading tracking-tight text-foreground">Add Product Type</h1>
                <p class="text-muted-foreground text-sm">Create a new product type.</p>
            </div>
        </div>

        <!-- Form Card -->
        <div class="rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm shadow-sm overflow-hidden p-6">
            <form action="{{ route('tenant.product_types.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name"
                        ...
                        placeholder="e.g. simple, variable, service" required value="{{ old('name') }}">
                ...
                <div class="pt-4 flex justify-end gap-3">
                    <x-ui.button type="button" variant="outline"
                        href="{{ route('tenant.product_types.index') }}">Cancel</x-ui.button>
                    <x-ui.button type="submit">Create Product Type</x-ui.button>
                </div>
            </form>
        </div>
    </div>
