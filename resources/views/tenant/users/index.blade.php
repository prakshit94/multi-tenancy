                    ...
                    <div x-cloak x-show="selected.length > 0" x-transition.opacity.duration.300ms
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.away="open = false"
                                class="group flex items-center gap-2 px-4 py-2 rounded-xl bg-background border border-input shadow-sm hover:shadow-md hover:border-primary/30 transition-all duration-200 text-sm font-medium">
                            ...
                            <div x-show="open" style="display: none;"
                                class="absolute left-0 top-full z-50 mt-2 min-w-[220px] p-1.5 rounded-xl border border-border/60 bg-popover/95 backdrop-blur-xl shadow-xl shadow-black/5 animate-in zoom-in-95 fade-in-0 slide-in-from-top-2">
                                <form action="{{ route($routePrefix . '.users.bulk') }}" method="POST">
                                    @csrf
                                    ...
                                    @if(request('trashed') === 'only')
                                        <button type="submit" name="action" value="restore"
                                            class="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-sm font-medium transition-colors hover:bg-emerald-500/10 hover:text-emerald-600 text-muted-foreground">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                stroke-linejoin="round">
                                                <path d="M3 7v6h6" />
                                        ...
                                        <button type="submit" name="action" value="force_delete"
                                            onclick="return confirm('This action is permanent! Are you sure?')"
                                            class="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-sm font-medium transition-colors hover:bg-destructive/10 hover:text-destructive text-muted-foreground">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                stroke-linejoin="round">
                                                <path d="M3 6h18" />
                                    ...
                                    @else
                                        ...
                                        <div
                                            class="px-2 py-1 text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider">
                                            Danger</div>
                                        <button type="submit" name="action" value="delete"
                                            onclick="return confirm('Move to trash?')"
                                            class="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-sm font-medium transition-colors hover:bg-destructive/10 hover:text-destructive text-destructive/80">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                stroke-linejoin="round">
                                                <path d="M3 6h18" />
                ...
                <div
                    ...
                    <div class="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        @if(request('search'))
                            <span class="flex items-center gap-1 ml-2 px-2 py-0.5 rounded-full bg-primary/10 text-primary">
                                ...
                                <a href="{{ url()->current() }}" class="hover:text-primary/80"><svg
                    ...
                    <div class="flex items-center gap-3">
                        <form id="per-page-form" method="GET" action="{{ url()->current() }}"
                            ...
                            @endif
                            @if(request('trashed')) <input type="hidden" name="trashed" value="{{ request('trashed') }}">
                            @endif
                    ...
                    <table class="w-full caption-bottom text-sm">
                        ...
                        <tbody class="[&_tr:last-child]:border-0 text-sm">
                            @forelse($users as $user)
                                <tr
                                    class="group border-b border-border/40 transition-all duration-200 hover:bg-muted/40 data-[state=selected]:bg-muted/60 {{ $user->trashed() ? 'bg-destructive/5' : '' }}">
                                    <!-- Row Checkbox -->
                                    ...
                                    <td class="p-6 align-middle">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="relative flex shrink-0 overflow-hidden rounded-full h-10 w-10 ring-2 ring-background shadow-sm transition-transform group-hover:scale-105">
                                                <span
                                                    class="flex h-full w-full items-center justify-center rounded-full bg-gradient-to-br from-primary/10 to-primary/5 text-primary font-bold text-xs border border-primary/10">
                                    ...
                                    <td class="p-6 align-middle">
                                        @foreach($user->roles as $role)
                                            <span
                                                class="inline-flex items-center rounded-md border border-border bg-secondary/50 px-2.5 py-0.5 text-xs font-medium text-secondary-foreground transition-colors hover:bg-secondary">
                                                {{ $role->name }}
                                            </span>
                                        @endforeach
                                    </td>
                                    ...
                                    <td class="p-6 align-middle text-right">
                                        <div class="relative flex justify-end" x-data="{ open: false }"
                                            ...
                                            <div x-show="open" x-transition:enter="transition ease-out duration-150"
                                                ...
                                                @if($user->trashed())
                                                    <form action="{{ route($routePrefix . '.users.restore', $user->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        <button type="submit"
                                                            class="w-full flex cursor-pointer select-none items-center gap-2 rounded-lg px-2 py-2 text-sm outline-none transition-colors hover:bg-emerald-500/10 hover:text-emerald-600 focus:bg-accent focus:text-accent-foreground">
                                                    ...
                                                    <form action="{{ route($routePrefix . '.users.destroy', $user->id) }}"
                                                        method="POST" onsubmit="return confirm('Permanently delete?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="w-full flex cursor-pointer select-none items-center gap-2 rounded-lg px-2 py-2 text-sm outline-none transition-colors hover:bg-destructive/10 hover:text-destructive focus:bg-destructive/10">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                                viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                                stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                                <path d="M3 6h18" />
                                ...
                                <tr>
                                    <td colspan="6" class="p-16 text-center">
                                        <div class="flex flex-col items-center justify-center text-muted-foreground/50">
                                            <div
                                                class="h-16 w-16 rounded-full bg-muted/30 flex items-center justify-center mb-4 ring-1 ring-border/50">
                                            ...
                                            <p class="text-sm mt-1 max-w-xs mx-auto">Try adjusting your safe terms or add a new
                                                user to get started.</p>
                                            @if(request('search'))
                                                <a href="{{ url()->current() }}"
                                                    class="mt-4 text-primary text-sm font-medium hover:underline">Clear Filters</a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                ...
                @if($users->hasPages() || $users->total() > 5)
                    <div
                        ...
                        <div class="text-xs text-muted-foreground px-2">Page <span
                                class="font-medium text-foreground">{{ $users->currentPage() }}</span> of <span
                                class="font-medium">{{ $users->lastPage() }}</span></div>
