@php
    $stats = [
        ['label' => 'Awaiting Confirmation', 'status' => 'confirmed', 'count' => $counts['confirmed'] ?? 0, 'color' => 'blue', 'icon' => '<circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/>'],
        ['label' => 'In Packing', 'status' => 'processing', 'count' => $counts['processing'] ?? 0, 'color' => 'purple', 'icon' => '<path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>'],
        ...
        ['label' => 'Delivered', 'status' => 'delivered', 'count' => $counts['delivered'] ?? 0, 'color' => 'green', 'icon' => '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>'],
    ];
@endphp
            ...
            @foreach($tabs as $tab)
                ...
                <a href="{{ route('tenant.processing.orders.index', array_merge(request()->query(), ['status' => $tab['status'], 'page' => null])) }}"
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <span class="text-[8px] font-bold mt-0.5 opacity-80 {{ $isActive ? 'text-white' : 'text-gray-500' }}">
                            ₹{{ number_format($amounts[$tab['status'] ?? 'active'] ?? 0, 0) }}
                        </span>
                    </div>
                </a>
            @endforeach
