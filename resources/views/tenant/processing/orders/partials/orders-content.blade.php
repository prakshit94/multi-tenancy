...
<div
    ...
    <div class="flex items-center gap-1 overflow-x-auto no-scrollbar w-full xl:w-auto">
        @php
            $navItems = [
                ['label' => 'Active', 'route' => route('tenant.processing.orders.index'), 'active' => !request('status'), 'count' => $counts['active'] ?? 0],
                ['label' => 'Confirmed', 'route' => route('tenant.processing.orders.index', ['status' => 'confirmed']), 'active' => request('status') === 'confirmed', 'count' => $counts['confirmed'] ?? 0],
                ['label' => 'Processing', 'route' => route('tenant.processing.orders.index', ['status' => 'processing']), 'active' => request('status') === 'processing', 'count' => $counts['processing'] ?? 0],
                ['label' => 'Ready to Ship', 'route' => route('tenant.processing.orders.index', ['status' => 'ready_to_ship']), 'active' => request('status') === 'ready_to_ship', 'count' => $counts['ready_to_ship'] ?? 0],
                ['label' => 'Dispatched', 'route' => route('tenant.processing.orders.index', ['status' => 'shipped']), 'active' => request('status') === 'shipped', 'count' => $counts['shipped'] ?? 0],
                ['label' => 'Delivered', 'route' => route('tenant.processing.orders.index', ['status' => 'delivered']), 'active' => request('status') === 'delivered', 'count' => $counts['delivered'] ?? 0],
                ['label' => 'Cancelled', 'route' => route('tenant.processing.orders.index', ['status' => 'cancelled']), 'active' => request('status') === 'cancelled', 'count' => $counts['cancelled'] ?? 0],
                ['label' => 'All History', 'route' => route('tenant.processing.orders.index', ['status' => 'all']), 'active' => request('status') === 'all', 'count' => $counts['all'] ?? 0],
            ];
        @endphp
            ...
            <div x-show="open" style="display: none;"
                ...
                <form action="{{ url()->current() }}" method="GET" class="space-y-2" x-data="{
                                submitFilter(e) {
                                    ...
                                    // Make sure we capture which button routed the form if it was a button click
                                    ...
                                    // Process form data into URL params
                                    ...
                                    // Re-use the loadData function from the search container
                                    // We need to dispatch an event since loadData is in a sibling component usually, or we can just fetch here
