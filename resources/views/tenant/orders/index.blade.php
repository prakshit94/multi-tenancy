...
@section('content')
    <div class="flex flex-1 flex-col space-y-8 p-6 md:p-8 max-w-7xl mx-auto w-full animate-in fade-in duration-500">
            ...
            <div class="flex items-center gap-3">
                <div class="flex items-center p-1 bg-muted/50 rounded-xl border border-border/50 backdrop-blur-sm">
                    <a href="{{ route('tenant.orders.index') }}"
                        class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ !request('status') ? 'bg-background text-foreground shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-foreground' }}">
                        All
                    </a>
                    <a href="{{ route('tenant.orders.index', ['status' => 'pending']) }}"
                        class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === 'pending' ? 'bg-background text-amber-600 shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-amber-600' }}">
                        Pending
                    </a>
                    <a href="{{ route('tenant.orders.index', ['status' => 'completed']) }}"
                        class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === 'completed' ? 'bg-background text-emerald-600 shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-emerald-600' }}">
                        Completed
                    </a>
                    <a href="{{ route('tenant.orders.index', ['status' => 'scheduled']) }}"
                        class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === 'scheduled' ? 'bg-background text-indigo-600 shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-indigo-600' }}">
                        Scheduled
                    </a>
                </div>
        ...
        <div
            ...
            <div
                class="p-4 border-b border-border/40 bg-muted/20 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2" x-data="{ open: false }">
                    <button @click="open = !open"
                        class="inline-flex items-center gap-2 rounded-lg border border-input bg-background px-3 py-2 text-xs font-medium hover:bg-accent transition-colors">
                    ...
                    <div x-show="open" @click.away="open = false"
                        class="absolute right-0 mt-32 w-48 rounded-xl border border-border bg-popover p-1 shadow-xl z-50">
                        <form action="{{ route('tenant.orders.export') }}" method="POST">
                            @csrf
                            <button name="format" value="csv"
                                class="w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-accent transition-colors">Export
                                CSV</button>
                            <button name="format" value="xlsx"
                                class="w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-accent transition-colors">Export
                                Excel (.xlsx)</button>
                            <button name="format" value="pdf"
                                class="w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-accent transition-colors">Export
                                PDF</button>
                        </form>
                    </div>
                            ...
                            <tr class="group hover:bg-muted/30 transition-colors">
                                <td class="px-6 py-4 font-mono font-medium text-foreground">
                                    <a href="{{ route('tenant.orders.show', $order) }}"
                                        class="font-semibold text-primary hover:underline">{{ $order->order_number }}</a>
                                    @if($order->status === 'shipped' && $order->shipments->isNotEmpty())
                                        <div class="text-[10px] font-mono text-muted-foreground/80 tracking-tighter"
                                            title="Tracking ID">{{ $order->shipments->first()->tracking_number }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-[10px]">
                                            {{ substr($order->customer->first_name ?? 'G', 0, 1) }}{{ substr($order->customer->last_name ?? '', 0, 1) }}
                                        </div>
                                        <div class="flex flex-col">
                                            <a href="{{ $order->customer_id ? route('tenant.customers.show', $order->customer_id) : '#' }}"
                                                class="font-medium text-foreground hover:text-primary hover:underline transition-colors">{{ $order->customer->first_name ?? 'Guest' }}
                                                {{ $order->customer->last_name ?? '' }}</a>
                                            <span
                                                class="text-[10px] text-muted-foreground">{{ $order->warehouse->name ?? 'Default' }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    @php
                                        ...
                                        if (in_array($order->status, ['completed', 'delivered'])) {
                                            $displayStatus = $order->payment_status === 'paid' ? 'completed' : 'delivered';
                                        }
                                        $statusClasses = [
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold border {{ $statusClasses[$displayStatus] ?? 'bg-muted text-muted-foreground' }}">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <div class="relative" x-data="{ open: false }" @click.away="open = false">
                                            <button @click="open = !open"
                                                class="p-2 rounded-lg hover:bg-accent text-muted-foreground transition-all">
                                            ...
                                            <div x-show="open"
                                                ...
                                                @if(!in_array($order->status, ['completed', 'delivered', 'cancelled', 'returned']))
                                                    <a href="{{ route('tenant.orders.edit', $order) }}"
                                                        class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-md border-t pt-2 mt-1">
                                                        Edit Order
                                                    </a>
                                                @endif

                                                @if(in_array($order->status, ['completed', 'delivered']))
                                                    <a href="{{ route('tenant.returns.create', ['order_id' => $order->id]) }}"
                                                        class="flex items-center gap-2 px-3 py-2 text-sm text-orange-600 hover:bg-orange-50 rounded-md border-t pt-2 mt-1">
                                                        Request Return
                                                    </a>
                                                @endif
                                            </div>
